...
public class Client extends JFrame {
    ...
    private void chargerCategories() {
        try {
            File xmlFile = new File("/Volumes/SANDISK/AP/Mission1/src/com/btssio/graphique/categorie.xml"); // Assurez-vous que ce fichier se trouve dans le même répertoire que votre code source.
            ...
            XPathExpression expr = xpath.compile("/categories/categorie");

            NodeList nodes = (NodeList) expr.evaluate(document, XPathConstants.NODESET);

            for (int i = 0; i < nodes.getLength(); i++) {
                String nom = nodes.item(i).getTextContent();
                categorieComboBox.addItem(new Categorie(i, nom, "", 0, 0));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void sauvegarderAdherents() {
        try (PrintWriter writer = new PrintWriter("/Volumes/SANDISK/AP/Mission1/adherents.txt")) {
            for (String adherent : valeurs) {
    ...
    private void chargerAdherents() {
        try (Scanner scanner = new Scanner(new File("/Volumes/SANDISK/AP/Mission1/adherents.txt"))){
    ...
    private void remplirJComboBox() {
        adherentsComboBox.removeAllItems();
    ...
    public Client() {
        ...
        categorieComboBox.setBounds(212, 466, 150, 25);
        contentPane.add(categorieComboBox);

        JButton btn_valider = new JButton("Valider");
        btn_valider.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                Categorie selectedCategorie = (Categorie) categorieComboBox.getSelectedItem();

                if (selectedCategorie == null) {
                    JOptionPane.showMessageDialog(Client.this, "Veuillez sélectionner une catégorie.");
                    return;
                }
                ...
                String pratique = (loisir.isSelected()) ? "loisir" : "compétition";
                String lateralite = (droitier.isSelected()) ? "droitier" : "gaucher";
                String genre = (chckbx_masculin.isSelected()) ? "masculin" : "féminin";

             // Nettoyer le nom de la catégorie
                String nomCategorie = selectedCategorie.getNom().replaceAll("\n", " ").trim();
                
                String adherentInfo = nom + " " + prenom 
                        + " Date de naissance : " + dateNaiss + " Pays et ville de naissance : " + paysVilleNaiss
                        + " Nationalité : " + nationalite + " Adresse : " + adresse + " Code postal : " + codePostal
                        + " Ville : " + ville + " Téléphone 1 : " + tel1 + " Téléphone 2 : " + tel2 + " Courriel : "
                        + courriel + " Nom et prénom du responsable : " + nomPrenomResponsable + " Pratique : " + pratique
                        + " Latéralité : " + lateralite + " Genre : " + genre
                		+ " Catégorie : " + nomCategorie;
                
                valeurs.add(adherentInfo);
                sauvegarderAdherents();
            }
        });
